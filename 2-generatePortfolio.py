...
def main():
    ...
    [risk_free, fit_frequency, portfolio_nbr] = [*config['Portfolio'].values()]
    ...
    # local params
    db_con = config['MySQL']
    engine = create_engine("mysql://%s:%s@%s/%s" % (db_con['user'], db_con['passwd'], db_con['host'], db_con['db']))
    ...
    if os.path.exists(nav_file['res_path']):
        ...
        nav = pd.read_sql_table('fund_nav', engine)
        cur = pd.read_sql_table('fund_nav_currency', engine)
        ...
        nav_model = nav_model.iloc[:-1, :]  # until last period

        # dump
        pk.dump(nav_model, open(nav_file['res_path'], 'wb'), True)

    nav_model = nav_model.sample(frac=0.1, random_state=1)

    # gen precise corvariance
    preCor = preciseCorvariance(nav_model)

    # gen return of interest
    return_vec = fitHM(nav_model, fit_frequency, 10)  # data, resample Frequency, years to go back
